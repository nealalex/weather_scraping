...
if not os.path.exists(RAW_DATA_DIR) or not os.listdir(RAW_DATA_DIR):
    ...
    print(
        "Please run '02_scraper.py' first to download the data, then run this script again."
    )
    exit()
...
for station_dir in os.listdir(RAW_DATA_DIR):
    ...
    if os.path.isdir(full_station_path):
        ...
        for f in station_files:
            try:
                df = pd.read_csv(f)
                # Add a check to ensure the file was scraped correctly
                if "Year" not in df.columns:
                    print(f"\n--- FATAL ERROR in {os.path.basename(f)} ---")
                    print(
                        "The 'Year' column is missing. This usually means the file was created"
                    )
                    print("by a scraper with an incorrect 'skiprows' value.")
                    print(
                        "Please correct the scraper, delete the contents of 'data/raw', and re-run the pipeline."
                    )
                    exit()
